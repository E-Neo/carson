...
pub struct Hub {
    sessions: Mutex<HashMap<String, Vec<UnboundedSender<SseItem>>>>,
}
...
impl Hub {
    ...
    pub fn register(&self, session_id: &str, tx: UnboundedSender<SseItem>) {
        self.sessions
            .lock()
            .unwrap()
    ...
    pub fn unregister(&self, session_id: &str, tx: &UnboundedSender<SseItem>) {
    ...
    pub fn send(&self, session_id: &str, item: SseItem) -> bool {
        let mut sessions = self.sessions.lock().unwrap();
...
mod tests {
    ...
    fn register_send_alive_unregister() {
        ...
        assert!(!hub.alive("s1"));
        hub.register("s1", tx.clone());
        assert!(hub.alive("s1"));
        assert!(hub.send("s1", item("chunk")));
    ...
    fn send_broadcasts_to_all_subscribers() {
        ...
        hub.register("s3", tx_a);
        hub.register("s3", tx_b);
        assert!(hub.send("s3", item("chunk")));
    ...
    fn unregister_removes_only_matching_channel() {
        ...
        let (tx_b, mut rx_b) = tokio::sync::mpsc::unbounded_channel();
        hub.register("s5", tx_a.clone());
        hub.register("s5", tx_b.clone());
        hub.unregister("s5", &tx_a);
        assert!(hub.alive("s5"));
        assert!(hub.send("s5", item("chunk")));
        assert_eq!(rx_b.try_recv().unwrap().event, "chunk");
        hub.unregister("s5", &tx_b);
        assert!(!hub.alive("s5"));
    }
    ...
    fn send_to_unregistered_session_fails() {
        let hub = Hub::new();
        assert!(!hub.send("s42", item("chunk")));
    }
    ...
    fn send_prunes_dropped_receivers() {
        ...
        hub.register("s9", tx_a);
        hub.register("s9", tx_b);
        drop(rx_b);
        assert!(hub.send("s9", item("chunk")));
        assert_eq!(rx_a.try_recv().unwrap().event, "chunk");
        assert!(hub.alive("s9"));
    }
}
